...
# ---------- v1.4 OMNICHANNEL PEOPLE-FIRST ----------
OMNI_CHANNELS = ["Threads", "YouTube Shorts", "Instagram", "Naver Blog", "Google Blog"]

CREATOR_BUSINESS_PRINCIPLES = """
MORROWAY의 수익화 원칙:
1) 가치 먼저, 판매는 나중. 무료 콘텐츠만으로도 독자가 얻어가는 것이 있어야 한다.
2) 플랫폼별 네이티브 편집. 같은 원고를 그대로 복붙하지 않는다.
3) 발견 채널(Threads/Shorts/Instagram) → 검색자산(Naver/Google) → 신뢰/관계 → 수익의 순서를 기본으로 한다.
4) 수익은 광고·플랫폼수익·제휴·브랜드협업·자체상품으로 분산한다. 한 가지에 과도하게 의존하지 않는다.
5) 건강·역사·인권·피해 사건은 불안·분노를 상품 판매에 이용하지 않는다. 필요하면 '판매 없음'을 선택한다.
6) 반응 없는 글까지 전부 장문·영상으로 확장하지 않는다. 반응 좋은 소재를 자산화한다.
7) Google/Naver 검색용 글은 AI 대량생산이 아니라 직접 경험·원자료·사진·표·체크리스트 등 고유가치를 추가한다.
""".strip()

OMNI_MARKERS = [
    "EDITOR_NOTE", "VALUE_CHECK", "THREADS", "FIRST_REPLY", "SHORTS", "SHORTS_SHOTS", "INSTAGRAM",
    "NAVER", "GOOGLE", "IMAGE_1X1", "IMAGE_9X16", "IMAGE_BLOG", "MONEY_ROUTE", "FACT_CHECK"
]

MORROWAY_EDITORIAL_DNA = """
MORROWAY 편집헌장:
- 사람의 존엄이 조회수보다 앞선다.
- 민주주의는 '내 편만 말하는 것'이 아니라 사실에 근거해 서로 다른 사람이 말할 자리를 지키는 태도로 구현한다.
- 평등, 공동체, 나보다 우리, 낮은 곳의 시선, 평범한 사람과 평범한 노동의 가치를 잊지 않는다.
- 약자·피해자·노동자·돌봄받는 사람을 동정의 소재나 소비 가능한 캐릭터로 만들지 않는다.
- 정치·역사·사회처럼 관점이 갈리는 주제는 사실/해석/의견을 분리하고, 정당한 이견을 허수아비로 만들지 않는다.
- 재미는 누군가를 깎아내리는 조롱보다 관찰·언어감각·뜻밖의 연결에서 만든다.
- 세대를 갈라 클릭을 얻지 않는다. 젊은 독자에게는 새롭고, X세대에게는 반갑고, 고연령 독자에게는 존중받는 느낌을 준다.
- '이 글을 읽고 사람이 무엇을 조금 더 이해하게 되는가?'가 모든 콘텐츠의 최종 질문이다.
""".strip()

EDITORIAL_MODE_GUIDES = {
    "설명형 에디토리얼": "큰 질문 하나를 던지고, 핵심 원인을 2~4개의 명료한 블록으로 설명한다. 문장은 짧고 정보 위계가 선명하다. 결론은 왜 이것이 우리의 일상에 중요한지로 돌아온다.",
    "그리움+궁금증": "낡은 사물·거리·소리·냄새·버스표·카세트·학교·골목 같은 구체적 감각에서 시작해 '왜 우리는 이것을 기억할까?'라는 질문으로 확장한다. 추억팔이로 끝내지 말고 현재의 삶과 연결한다.",
    "생활교양 위트": "일상에서 한 번쯤 겪는 장면을 정확하게 집어내고, 웃기려 애쓰지 않은 관찰형 한 문장으로 미소를 만든 뒤 실제 쓸모를 준다.",
    "사람·사회·공공": "제도나 사건을 추상적 구호보다 평범한 사람의 하루와 연결해 설명한다. 공공성·민주적 절차·노동·돌봄·평등의 의미를 선동 없이 구체적으로 보여준다.",
    "쓸모형": "독자가 오늘 바로 써먹을 선택 기준·체크리스트·주의점을 우선한다. 상품보다 기준, 공포보다 판단력을 준다."
}

VISUAL_MODE_GUIDES = {
    "설명+그리움 하이브리드": "현대 에디토리얼 explainer의 대담한 정보 위계 + 빈티지 종이 콜라주. 크림 배경, 딥 네이비, 청록, 코랄. 오래된 생활물건 하나를 중심 오브젝트로 두고 간결한 도형·파형·지도 조각·메모를 섞는다. '무슨 이야기지?'와 '아, 그때'가 동시에 느껴져야 한다.",
    "에디토리얼 explainer": "대담한 산세리프 타이포를 얹을 넓은 여백, 단순 도형, 명확한 시선 흐름, 한 가지 강한 시각 은유. 특정 언론사의 로고·레이아웃·고유 그래픽을 복제하지 않고 독창적 구성을 쓴다.",
    "노스탤지어 콜라주": "빛바랜 인쇄물·버스표·카세트·공중전화·학교 노트·골목 표지판 등 시대를 암시하는 일반 사물을 종이 콜라주처럼 배치한다. 특정 브랜드/저작물은 피하고 감각적 기억을 불러온다.",
    "미니멀 인포그래픽": "크림 바탕 위 2~4개의 핵심 숫자/도식/아이콘을 정돈한다. 의료·안전·제도 정보처럼 정확성이 중요한 주제에 적합하며 장식보다 이해를 우선한다."
}

GENERATION_GUIDES = {
    "세대 연결형": "20대는 새롭게 배우고, 40~50대는 자기 기억을 꺼내고, 60대 이상은 경험이 존중받도록 세대 간 다리를 놓는다.",
    "조금 더 젊게": "낡아 보이는 표현과 훈계조를 줄이고 질문·장면·짧은 문장으로 시작한다. 다만 억지 유행어는 쓰지 않는다.",
    "조금 더 그리운 쪽": "구체적인 물건·공간·소리를 더 활용하되 '그때가 무조건 좋았다'는 미화는 피한다."
}

def omnichannel_free_prompt(topic, category, purpose, target_date, experience_note="", current_issue="", source_url="", channels=None, monetization_intent="일단 정보와 신뢰", editorial_mode="설명형 에디토리얼", visual_mode="설명+그리움 하이브리드", generation_bridge="세대 연결형"):
    channels = channels or OMNI_CHANNELS
    ch = ", ".join(channels)
    sensitive = category in ["오늘의 몸", "오늘의 역사", "오늘의 이슈", "오늘의 돌봄"]
    verify = "건강·역사·최신 사실은 웹에서 확인하고 공신력 높은 1차/공식 출처를 우선하라." if sensitive or current_issue or source_url else "최신 사실이 필요하면 기억에 의존해 단정하지 말고 확인하라."
    editorial_guide=EDITORIAL_MODE_GUIDES.get(editorial_mode,EDITORIAL_MODE_GUIDES["설명형 에디토리얼"])
    visual_guide=VISUAL_MODE_GUIDES.get(visual_mode,VISUAL_MODE_GUIDES["설명+그리움 하이브리드"])
    generation_guide=GENERATION_GUIDES.get(generation_bridge,GENERATION_GUIDES["세대 연결형"])
    return f"""너는 생활교양 미디어 '오늘도 한살 | MORROWAY'의 편집장이다.
오늘 날짜: {target_date}
핵심 주제: {topic}
카테고리: {category}
이번 콘텐츠의 1차 역할: {purpose}
글의 결: {editorial_mode}
이미지 결: {visual_mode}
세대 감성: {generation_bridge}
내 경험·관찰: {experience_note or '없음'}
오늘 본 이슈: {current_issue or '없음'}
참고 URL: {source_url or '없음'}
필요 채널: {ch}
수익 연결 의도: {monetization_intent}
...
브랜드 문체:
- 따뜻하고 지적이지만 선생님처럼 훈계하지 않는다.
- 첫 문장은 호기심·장면·의외성 중 하나로 짧게 시작한다.
- 첫 2~4문장 안에 훅의 약속을 갚는다.
- 어려운 전문용어보다 쉬운 비유, 구체적 숫자, 생활 장면을 쓴다.
- 관찰형 위트는 한 꼬집. 인권·참사·피해·중증질환에는 위트를 억제한다.
- 마지막은 구호가 아니라 독자의 기억·선택·사람 관계로 돌아오는 한 문장 또는 자연스러운 질문으로 끝낸다.
- 정보+재미+가치 셋 중 최소 둘, 가능하면 셋을 모두 충족한다.
- 사람을 불안하게 하거나 화나게 만들어 클릭·구매시키지 않는다.
- 사실/해석/의견을 구분한다. {verify}

{CREATOR_BUSINESS_PRINCIPLES}

플랫폼별 편집 원칙:
[Threads] 180~430자 중심. 첫줄은 스크롤을 멈추게 하되 낚시 금지. 자기 경험·기억·지역·세대의 말을 꺼내기 쉬운 질문 1개. 본문과 별도로 첫 댓글 1개를 설계한다.
[YouTube Shorts] 35~55초. 2초 훅 → 장면/정보 3~5비트 → 여운. 원본성이 느껴지는 별도 대본으로 재편집하고 다른 채널 문장을 읽기만 하는 재활용 영상은 피한다.
[Instagram] 릴스 또는 저장형 6장 카드뉴스 중 더 맞는 형식을 고른다. 비주얼이 먼저 이해되고 캡션은 이미지 반복이 아니라 맥락·경험을 보탠다.
[Naver Blog] 한국어 검색 질문에 빠르게 답하고, 실제 경험·공공자료·체크리스트·직접 사진 중 최소 2개를 더할 자리를 표시한다.
[Google Blog] 사람우선 검색자산. 직접 경험·원자료·사진/표·독자용 도구 중 최소 2개를 보태야 발행 가능. 대량 자동생성/키워드 변형 페이지 금지.

수익화 판단:
- '사람 모으기/신뢰 쌓기/공공가치' 소재면 판매 없음이 기본값일 수 있다.
- 제품 선택 소재면 제품명보다 선택 기준을 먼저 제시한다.
- 장기적으로 플랫폼 광고, 자연스러운 제휴, 브랜드 협업, 자체 PDF/전자책/체크리스트 중 가장 어울리는 1~2개만 선택한다.
- 건강 불안, 참사, 인권 피해, 정치적 분노를 구매 전환 장치로 쓰지 않는다.

이미지 제작 원칙:
- {visual_guide}
- 현대 설명형 미디어에서 느껴지는 명확한 정보 위계·대담한 타이포 공간·콜라주 감각은 참고하되 특정 매체의 로고, 고유 레이아웃, 대표 일러스트를 복제하지 않는다.
- ORIGINAL 시각물. 크림색 바탕 + 딥 네이비 + 청록 + 작은 코랄 포인트를 MORROWAY 기본 팔레트로 쓴다.
- 그리움이 맞는 소재면 특정 시대를 상징하는 일반 사물 하나(카세트, 버스표, 공중전화, 교복 가방, 오래된 주방도구, 골목 표지 등)를 중심 오브젝트로 삼는다.
- 궁금증이 중요한 소재면 '왜?'를 시각적으로 암시하는 빈칸·대조·부분 확대·지도 조각·파형·도형을 활용한다.
- 이미지 생성 모델에는 한국어 본문을 많이 쓰게 하지 말고, 제목을 넣을 안전한 여백을 확보하도록 지시한다.
- 인물은 특정 실존인을 닮게 만들지 않고 존엄하게 표현한다.

아래 마커를 정확히 유지해서 결과를 작성해줘. 마커 사이에는 실제 게시 가능한 원고만 넣는다.
<<<EDITOR_NOTE>>>
오늘 이 주제를 왜 다루는지 + 사람에게 주는 가치 + 판매를 붙여도 되는지 4줄 이내.
<<<VALUE_CHECK>>>
다음 6항목을 '좋음/보완/주의'로 짧게 점검: 정보 정확성, 재미, 사람 존중, 세대 연결, 대화 가능성, 상업성 과잉. 마지막에 '이 글이 누군가를 조금 더 이해하게 만드는가?' 한 줄 판정.
<<<THREADS>>>
Threads 완성본. 첫줄 포함. 마지막 질문은 자연스러울 때만 1개.
<<<FIRST_REPLY>>>
작성자가 자기 글 아래 먼저 달 첫 댓글 1개. 보충정보/경험/출처 중 가장 좋은 형태.
<<<SHORTS>>>
35~55초 완성 대본. 내레이션 문장만.
<<<SHORTS_SHOTS>>>
6~9컷 화면/자막 지시. 저작권 없는 자료 또는 직접 제작 가능한 화면 위주.
<<<INSTAGRAM>>>
릴스 또는 6장 카드뉴스 중 하나를 고르고, 장면/슬라이드별 문구 + 캡션까지.
<<<NAVER>>>
네이버 블로그 제목 3개 + 리드 + 소제목 구조 + 본문 초안 + 독자가 저장할 체크리스트. 사람이 보탤 경험/사진/공공자료 자리 표시.
<<<GOOGLE>>>
Google 검색용 제목 3개 + 메타설명 + 사람우선 장문 구조/초안. 직접 경험·원자료·표/사진 자리 표시. 대량 자동 발행 금지 검수 메모 포함.
<<<IMAGE_1X1>>>
Threads/Instagram 1:1 커버 이미지 생성 프롬프트. 구성·소품·질감·시선 흐름·색·제목 안전영역을 구체적으로. 이미지 안 긴 한국어 문장 생성 금지.
<<<IMAGE_9X16>>>
Shorts/Reels 9:16 커버/배경 이미지 생성 프롬프트. 첫 2초에 이해되는 중심 오브젝트와 상하 자막 안전영역 포함.
<<<IMAGE_BLOG>>>
Naver/Google용 16:9 또는 3:2 대표 이미지 생성 프롬프트. 검색 결과 썸네일에서도 의미가 살아야 한다.
<<<MONEY_ROUTE>>>
판매 없음 / 플랫폼 광고 / 자연스러운 제휴 / 브랜드 협업 / 자체상품 중 추천 1~2개와 이유. '지금은 팔지 말자'도 정답으로 인정.
<<<FACT_CHECK>>>
게시 전 확인할 사실 0~5개 + 필요한 출처 유형. 사실확인이 거의 필요 없으면 '특별 확인 없음'.
"""

def parse_omni_pack(text:str):
    import re
    out={}
    if not text: return out
    pattern=r"<<<([A-Z0-9_]+)>>>(.*?)(?=<<<[A-Z0-9_]+>>>|\Z)"
    for key,body in re.findall(pattern,text,flags=re.S):
        out[key]=body.strip()
    return out

def omni_pack_to_text(sections:dict):
    parts=[]
    for key in OMNI_MARKERS:
        if sections.get(key): parts.append(f"<<<{key}>>>\n{sections[key].strip()}")
    return "\n\n".join(parts)

def call_text(prompt:str, model:str, api_key=None):
    c=client_for(api_key)
    r=c.responses.create(model=model,reasoning={"effort":"low"},input=prompt)
